...
loglik_vertical_CML <- function(theta, sigma, my.data.full){
  ...
  for(j in unique(my.data.full$Period)){
    treated_list.j <- my.data.full$mu.ij[my.data.full$Period == j & my.data.full$Interv == 1]
    control_list.j <- my.data.full$mu.ij[my.data.full$Period == j & my.data.full$Interv == 0]
...
loglik_vertical_CML_adj <- function(theta, sigma, betabase, my.data.full){
  ...
  for(j in unique(my.data.full$Period)){
    ...
    treated_list_base.j <- my.data.full$mu.ij[my.data.full$Period == 1 &
                                                       my.data.full$Cluster %in% my.data.full$Cluster[my.data.full$Period == j & my.data.full$Interv == 1]]
    control_list_base.j <- my.data.full$mu.ij[my.data.full$Period == 1 &
...
CLWP_fit <- function(my.data, start_theta, start_sigma, N) {
  ...
  for(j in unique(my.data$Period)){
    treated_list.j <- my.data$mu.ij[my.data$Period == j & my.data$Interv == 1]
    control_list.j <- my.data$mu.ij[my.data$Period == j & my.data$Interv == 0]
    combos.j <- expand.grid(treated=treated_list.j,control=control_list.j)
